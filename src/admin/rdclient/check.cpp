...
BOOL IsAdminProgram()
{
  ...
  GetFileNameInLocalDir("settings.exe", s);
...
extern "C" __declspec(dllexport) BOOL __cdecl CheckAtStartup(HWND parent, BOOL silent)
{
  ...
  if (IsAdminProgram())
  {
    ...
    {
      if (!silent)
        MessageBox(parent, "Необходимы права администратора для запуска.", NULL, MB_OK | MB_ICONERROR);
  ...
  {
    if (!silent)
      MessageBox(parent, "Возможен запуск только из программы учителя.", NULL, MB_OK | MB_ICONERROR);
