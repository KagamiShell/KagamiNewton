...
void GetLnkDescription(const LNKINFO *li, char *desc, BOOL force_old)
{
  ...
  if (li->psl)
  {
    ...
    if (li->psld)
    {
      ...
      else if (li->psld->CopyDataBlock(LNK_DESC_ID, &buff) == S_OK)
      {
        if (buff)
        {
          if (*((DWORD *)buff + 0) > sizeof(DWORD) + sizeof(DWORD))
...
extern "C" __declspec(dllexport) void __cdecl OldLnkFile_Iterate(const char *path, OLDFILEPROC proc, void *parm)
{
  CoInitialize(0);
...
extern "C" __declspec(dllexport) HDEVINFO __cdecl DLL_CreateFlashDrivesEnumerator()
{
...
extern "C" __declspec(dllexport) void __cdecl DLL_DestroyFlashDrivesEnumerator(HDEVINFO h)
{
  if (h != INVALID_HANDLE_VALUE)
  {
...
extern "C" __declspec(dllexport) BOOL __cdecl DLL_GetNextFlashDrive(HDEVINFO h, int &idx, char *s_inst, int max)
{
  BOOL rc = FALSE;
