...
void CNRListCtrl::OnSize(UINT nType, int cx, int cy)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		{
		case 0:
			nWidth = (int)((COLUMN_1 * cx) / 100);
			break;
		case 1:
			nWidth = (int)((COLUMN_2 * cx) / 100);
			break;
		case 2:
			nWidth = (int)((COLUMN_3 * cx) / 100);
			break;
		}
...
void CNRListCtrl::OnLvnGetdispinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if (pItem->mask & LVIF_TEXT)
	{
		...
		else if (pItem->iSubItem == 1) //вторая колонка
		{
			ULONGLONG iSize = (*iter_drives)->iSizeFile;
			if ((*iter_drives)->bDirectory)
				str_tmp = "<DIR>";
			else if (m_b_view_only_files)
			{
				str_tmp = "---";
			}
			else
		...
		else if (pItem->iSubItem == 2) // третья колонка
		{
...
bool UDgreater(PDATA_FILE pData1, PDATA_FILE pData2)
{
	...
	case 1:
	{
		if (pData1->iSizeFile == pData2->iSizeFile)
			return false;
		else if (pData1->iSizeFile > pData2->iSizeFile)
			return ((CNRListCtrl::g_bSwitchSort) ? true : false);
		else
			return ((CNRListCtrl::g_bSwitchSort) ? false : true);
		break;
	...
	case 2:
	{
		if (pData1->timeCreate == pData2->timeCreate)
			return false;
		else if (pData1->timeCreate > pData2->timeCreate)
			return (CNRListCtrl::g_bSwitchSort) ? true : false;
		else
			return (CNRListCtrl::g_bSwitchSort) ? false : true;
		break;
