...
c_get_list_drive_cmd::c_get_list_drive_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
...
void c_get_list_drive_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	if (pPacket->c_code_err == 0)
	{
		...
		{
			char *p_packet = NULL;
			int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_GET_LIST_DRIVES, 0, m_i_size_recv_data, 0, NULL, &p_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
			if (p_packet)
				delete[] p_packet;
		}
	...
	else if (pPacket->c_code_err == ERR_TIMEOUT)
	{
	...
	else if (pPacket->c_code_err == ERR_LIST_DRIVES)
	{
...
c_get_list_files_cmd::c_get_list_files_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
	m_i_size_recv_data = 0;
}
...
bool c_get_list_files_cmd::get_list_files(CListFiles &list_files, CString sz_path, CString &sz_err_descr)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_GET_LIST_FILES, 0, 0L, sz_path.GetLength() + 1, sz_path.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_get_list_files_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	if (m_p_rfm_thread)
	{
		if (WaitForSingleObject(m_p_rfm_thread->get_handle_stop(), 0) == WAIT_OBJECT_0)
		{
			...
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_GET_LIST_FILES, ERR_ABORT_PACK, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
	...
	if (pPacket->c_code_err == 0)
	{
		...
		{
			char *p_packet = NULL;
			int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_GET_LIST_FILES, 0, m_i_size_recv_data, 0, NULL, &p_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
			if (p_packet)
				delete[] p_packet;
		}
	...
	else if (pPacket->c_code_err == ERR_LIST_FILES)
	{
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
		m_i_size_recv_data = 0;
		m_flag_finish = false;
...
c_delete_file_cmd::c_delete_file_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
}
...
bool c_delete_file_cmd::cmd_delete_file(CString sz_path, CString &sz_err_descr)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_DELETE_FILE, 0, 0L, sz_path.GetLength() + 1, sz_path.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_delete_file_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	else if (pPacket->c_code_err == ERR_DELETE_FILE)
	{
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
		m_flag_finish = false;
		::SetEvent(m_h_end_operation);
...
c_create_dir_cmd::c_create_dir_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
}
...
bool c_create_dir_cmd::cmd_create_dir(CString str_path, CString &sz_err_descr)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_CREATE_DIR, 0, 0L, str_path.GetLength() + 1, str_path.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_create_dir_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	else if (pPacket->c_code_err == ERR_CREATE_DIR)
	{
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
		m_flag_finish = false;
		::SetEvent(m_h_end_operation);
...
c_copy_file_in::c_copy_file_in(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
	m_h_file_copy = NULL;
}
...
bool c_copy_file_in::cmd_copy_file(CString sz_path_server, CString sz_path_local, CString &sz_err)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_IN, 0, 0L, sz_path_server.GetLength() + 1, sz_path_server.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_copy_file_in::ProcessCommand(char *pData, int nSize)
{
	...
	if (pPacket->c_code_err == 0)
	{
		...
		if (!iSizeFile)
		{
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_IN, 0, 1, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
		}
		...
		if (WaitForSingleObject(m_p_rfm_thread->get_handle_stop(), 0) == WAIT_OBJECT_0)
		{
			...
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_IN, ERR_ABORT_PACK, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
		...
		i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_IN, 0, iSizeFile, (unsigned short)read_data, ch_tmp_buf, &p_data_packet);
		((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
		if (p_data_packet)
			delete[] p_data_packet;
	...
	else if (pPacket->c_code_err == ERR_COPY_FILE)
	{
		CloseHandle(m_h_file_copy);
		m_h_file_copy = NULL;
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
...
c_copy_file_out::c_copy_file_out(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
	m_h_file_copy = NULL;
	m_liSize = 0;
	m_sz_path_file = "";
...
bool c_copy_file_out::cmd_copy_file(CString sz_path_server, CString sz_path_local, CString &sz_err)
{
	...
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_OUT, 0,
																	...
																	sz_path_server.GetLength() + 1, sz_path_server.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
...
void c_copy_file_out::ProcessCommand(char *pData, int nSize)
{
	...
	if (pPacket->c_code_err == 0)
	{
		if (m_liSize != pPacket->ui_total_size)
		{
			...
			size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_OUT, 0, 0, 0, NULL, &p_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
			if (p_packet)
				delete[] p_packet;
		...
		{
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_OUT, ERR_ABORT_PACK, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
			::CloseHandle(m_h_file_copy);
		...
		else
		{
			...
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COPY_OUT, 0, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
		}
...
c_move_file_in::c_move_file_in(CRfmClientSocket *pTransport) : c_copy_file_in(pTransport)
{
}
...
bool c_move_file_in::cmd_move_file(CString sz_path_server, CString sz_path_local, CString &sz_err)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_IN, 0, 0L, sz_path_server.GetLength() + 1, sz_path_server.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_move_file_in::ProcessCommand(char *pData, int nSize)
{
	...
	if (pPacket->c_code_err == 0)
	{
		...
		if (WaitForSingleObject(m_p_rfm_thread->get_handle_stop(), 0) == WAIT_OBJECT_0)
		{
			...
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_IN, ERR_ABORT_PACK, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
		...
		i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_IN, 0, iSizeFile, (unsigned short)read_data, ch_tmp_buf, &p_data_packet);
		((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
		if (p_data_packet)
			delete[] p_data_packet;
...
c_move_file_out::c_move_file_out(CRfmClientSocket *pTransport) : c_copy_file_out(pTransport)
{
}
...
void c_move_file_out::ProcessCommand(char *pData, int nSize)
{
	...
	if (pPacket->c_code_err == 0)
	{
		if (m_liSize != pPacket->ui_total_size)
		{
			...
			size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_OUT, 0, 0, 0, NULL, &p_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
			if (p_packet)
				delete[] p_packet;
		...
		if (!::WriteFile(m_h_file_copy, pdata_pack, pPacket->i_size_data, &dw_was_write, NULL))
		{
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_OUT, ERR_ABORT_PACK, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
			::CloseHandle(m_h_file_copy);
		...
		else
		{
			...
			i_size = ((c_TransportLevel *)m_pTransport)->make_packet(ID_MOVE_OUT, 0, 0, 0, NULL, &p_data_packet);
			((c_TransportLevel *)m_pTransport)->SendData(p_data_packet, i_size);
			if (p_data_packet)
				delete[] p_data_packet;
		}
...
c_rename_file_cmd::c_rename_file_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
}
...
bool c_rename_file_cmd::cmd_move_file(CString sz_path_old, CString sz_path_new, CString &sz_err)
{
	...
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_RENAME_FILE, 0, 0L, sz_data_path.GetLength() + 1, sz_data_path.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
...
void c_rename_file_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	else if (pPacket->c_code_err == ERR_RENAME_FILE)
	{
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
		m_flag_finish = false;
		::SetEvent(m_h_end_operation);
...
c_exec_file_cmd::c_exec_file_cmd(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
}
...
bool c_exec_file_cmd::cmd_exec_file(CString sz_path_server, CString &sz_err)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_EXEC_FILE, 0, 0L, sz_path_server.GetLength() + 1, sz_path_server.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
...
void c_exec_file_cmd::ProcessCommand(char *pData, int nSize)
{
	...
	else if (pPacket->c_code_err == ERR_EXEC_FILE)
	{
		m_sz_error_descr.Append(pdata_pack, pPacket->i_size_data);
		m_flag_finish = false;
		::SetEvent(m_h_end_operation);
...
c_count_files_dir::c_count_files_dir(CRfmClientSocket *pTransport) : c_LogicCommand(pTransport)
{
	m_i_count_files = 0;
}
...
UINT c_count_files_dir::cmd_count_files_dir(CString sz_path_server)
{
	...
	char *p_packet = NULL;
	int size_pack = ((c_TransportLevel *)m_pTransport)->make_packet(ID_COUNT_FILES_DIR, 0, 0L, sz_path_server.GetLength() + 1, sz_path_server.GetBuffer(), &p_packet);
	((c_TransportLevel *)m_pTransport)->SendData(p_packet, size_pack);
	if (p_packet)
		delete[] p_packet;
	// Ожидать события получения данных
