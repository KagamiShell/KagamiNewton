...
void CPicPreview::PrepareBuffer(const WCHAR *title)
{
  ...
  if ( title && title[0] )
     {
       HFONT font = CreateFontW(-30,0,0,0,FW_BOLD,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH,L"Segoe UI");
        ...
        if ( total_w > 0 && total_h > 0 && pic_w > 0 && pic_h > 0 )
           {
             ...
             if ( i->name && i->name[0] )
                {
                  HFONT font = CreateFontW(-12,0,0,0,FW_NORMAL,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH,L"Segoe UI");
  ...
  if ( g_pics.size() > 0 )
     {
       ...
       wc.lpszClassName = "_KNPicsPreviewWindowClass3";
       RegisterClass(&wc);
