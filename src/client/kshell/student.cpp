  ...
  if ( student_session[0] )
     {
       ...
       PostMessage(HWND_BROADCAST,RegisterWindowMessage("_KNStudentSessionEnd"),0,0);
  ...
  if ( login && login[0] && folder && folder[0] )
     {
       ...
       PostMessage(HWND_BROADCAST,RegisterWindowMessage("_KNStudentSessionBegin"),0,0);
       SendDynamicInfoToServer();
       ...
       if ( !IsStrEmpty(device_str) )
          {
            if ( IsWindowVisible(GetDlgItem(hwnd,IDC_EDIT3)) )
               ...
               { //login
                 ...
                 PostMessage(GetMainWnd(),RS_LOGINSTUDENTSESSION_NOFORCE,0,(LPARAM)GlobalAddAtom(device_str));
               }
          }
