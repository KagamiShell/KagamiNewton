...
{
	return BOOL2HRESULT( RS_StudentLogin(lpszLogin, lpszPassword, bWait) );
}

STDMETHODIMP CRunpadShell2::StudentRegister(LPCSTR lpszLogin, LPCSTR lpszPassword, BOOL bWait)
{
	return BOOL2HRESULT( RS_StuRegister(lpszLogin, lpszPassword, bWait) );
}

STDMETHODIMP CRunpadShell2::StudentLogout(BOOL bWait)
{
	return BOOL2HRESULT( RS_StudentLogout(bWait) );
}
