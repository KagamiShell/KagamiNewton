...
var
  ...
  student_begin_message: THandle;
  student_end_message: THandle;
...
{$R *.dfm}
{$INCLUDE ..\kn_shared\KN_Shared.inc}
...
begin
 if Msg.message=msg_langchanged then
  ...
  begin
    if IStudentFolder >= 0 then
     begin
      if Msg.message=student_begin_message then
        begin
          ComboBox.ItemsEx.Items[IStudentFolder].Caption := GetStudentFolderName(@buf);
          ListView.Items[IStudentFolder-1].Caption := GetStudentFolderName(@buf);
          Handled := True;
        end
      else if Msg.message=student_end_message then
        begin
          ComboBox.ItemsEx.Items[IStudentFolder].Caption := GetStudentFolderName(@buf);
          ListView.Items[IStudentFolder-1].Caption := GetStudentFolderName(@buf);
          if GetExploringPlace = SStudentFolder then
            ExploreDefault;
          Handled := True;
        end
     end;
...
begin
 if BodyExplForm.IsRemovableMedia then
   begin
     Result:=GetStudentFolderPath(@buf);
     if buf[0] = #0 then
...
begin
  ...
  if IsStudentFolderAllowed() then
   begin
    IStudentFolder := ComboBox.ItemsEx.Count;
    ComboBox.ItemsEx.AddItem(GetStudentFolderName(@buf), 4,4, -1, 1, @SStudentFolder);
   end;
...
begin
  ...
  if IsStudentFolderAllowed() then
   begin
    Item := ListView.Items.Add;
    Item.Caption := GetStudentFolderName(@buf);
    Item.Data := @SStudentFolder;
    Item.ImageIndex := 4;
   end;
...
begin
  ...
  else if (Pos(SStudentFolder, s) = 1) and (IStudentFolder>=0) then
    begin
      Root := GetStudentFolderPath(@buf);
      Title := GetStudentFolderName(@buf);
      ComboBox.ItemIndex := IStudentFolder;
    end
  ...
  if Root='' then
    begin
      ...
      if ComboBox.ItemIndex = IStudentFolder then
       begin
...
// except end;
 ...
 msg_langchanged:=RegisterWindowMessage('_KNLanguageChanged');
 student_begin_message := RegisterWindowMessage('_KNStudentSessionBegin');
 student_end_message := RegisterWindowMessage('_KNStudentSessionEnd');
...
begin
  WinRARPath := '';
  reg:=TRegistry.Create;
  if reg.OpenKeyReadOnly('Software\KagamineP\KagamiShell') then
    begin
      try
...
function CreateNewDOCX(s:string):boolean;
begin
 Result:=CreateNewEmptyDOCX(pchar(s));
end;

function CreateNewXLSX(s:string):boolean;
begin
 Result:=CreateNewEmptyXLSX(pchar(s));
end;
...
(
  ...
  (ext:'.docx';name:1401;func:CreateNewDOCX),
  (ext:'.xlsx';name:1402;func:CreateNewXLSX)
);
