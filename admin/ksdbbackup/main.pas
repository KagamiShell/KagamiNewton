...
// Тоже должен быть изменен в DBConf
const KAGAMI_BASE = 'KagamiShell';
const REGPATH = 'Software\KagamineP\KagamiShell\KagamiShellAdmin';
     ...
     if RadioBackup.Checked or RadioRestore.Checked then
      begin
       if RadioBackup.Checked then
        Caption:='Сохранение базы данных KagamiShell'
       else
        Caption:='Восстановление базы данных KagamiShell';
       idx:=1;
      end
     else
      begin
  ...
  begin
   ...
   if use_backup_login then
    begin
     ...
     q:='BACKUP DATABASE '+KAGAMI_BASE+' TO DISK='''+lib.EscapeString(s_file)+'''';
    ...
    begin
     q:='RESTORE DATABASE '+KAGAMI_BASE+' FROM DISK='''+lib.EscapeString(s_file)+''' WITH REPLACE';
     Result:=lib.Exec(q,300);
     if not Result then
      begin
       AddLog('Error: '+lib.GetLastError);
       AddLog('Убедитесь, что пользователь имеет права администратора БД и файл выбран верно');
       AddLog('Также необходимо временно удалить сервер KagamiShell и отключить другие подключения к базе KagamiShell перед выполнением операции!');
      end
     else
      begin
       if lib.Exec('USE '+KAGAMI_BASE) then
        begin
   ...
   if Result then
    begin
     ...
     if RadioRestore.Checked then
      begin
       //AddLog('');
       AddLog('Внимание!!! Не забудьте запустить утилиту конфигурации базы KagamiShell, а потом заново создать пользователей в программе глобальных настроек KagamiShell!!!');
      end;
    end;
  end
 else
