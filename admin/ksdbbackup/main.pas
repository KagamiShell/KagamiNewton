...
const REGPATH = 'Software\Team KagamineP\KagamiShellAdmin';
    ...
    begin
     ...
     if not Result then
      begin
       ...
       AddLog('Также необходимо временно удалить сервер Kagami и отключить другие подключения к базе Kagami перед выполнением операции!');
      end
     else
      begin
   ...
   if Result then
    begin
     ...
     if RadioRestore.Checked then
      begin
       //AddLog('');
       AddLog('Внимание!!! Не забудьте запустить утилиту конфигурации базы Kagami, а потом заново создать пользователей в программе глобальных настроек Kagami!');
      end;
    end;
  end
 else
