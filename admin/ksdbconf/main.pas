...
begin
  ...
  begin
    if MessageBox(Handle, pchar(
      'Необходимо выполнить подготовку базы после установки/обновления администраторской части!'#13#10#13#10'Отменить выполнение?'), 'Предупреждение', MB_YESNO or MB_DEFBUTTON2 or MB_ICONWARNING) <> IDYES then
      CanClose := false;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not RecurseAddUpdateFile(lib, stored_proc, IncludeTrailingPathDelimiter(search_path) +
          s_local, base_path, env) then
        begin
          Result := false;
          break;
        end;
