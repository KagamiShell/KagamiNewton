...
uses CommCtrl, ShellApi, tools, tip, global, lang, VistaAltFixUnit;
...
var col_services: array [0..10] of TCOLINFO =
    (
    (defwidth: 85; title: 'Время'; expr_ms: 'TServices.time';
    expr_my: ''),
    (defwidth: 100; title: 'Услуга'; expr_ms: 'TPrices.name';
    expr_my: ''),
    (defwidth: 100; title: 'Компьютер'; expr_ms: 'TServices.comp_desc';
    expr_my: 'CONCAT(TServices.comp_desc)'),
    (defwidth: 100; title: 'Имя (IP)'; expr_ms: 'TServices.comp_name+'' (''+TServices.comp_ip+'')''';
    expr_my: 'CONCAT(TServices.comp_name,'' ('',TServices.comp_ip,'')'')'),
    (defwidth: 100; title: 'Domain/User'; expr_ms: 'TServices.user_domain+''/''+TServices.user_name';
    expr_my: 'CONCAT(TServices.user_domain,''/'',TServices.user_name)'),
    ...
    (defwidth: 400; title: 'Описание'; expr_ms: 'TServices.comments';
    expr_my: ''));
...
var col_events: array [0..6] of TCOLINFO =
    ...
    (defwidth: 100; title: 'Зал/Машина'; expr_ms: 'comp_loc+''/''+comp_desc';
    expr_my: 'CONCAT(comp_loc,''/'',comp_desc)'),
    (defwidth: 100; title: 'Имя (IP)'; expr_ms: 'comp_name+'' (''+comp_ip+'')''';
    expr_my: 'CONCAT(comp_name,'' ('',comp_ip,'')'')'),
    (defwidth: 100; title: 'Domain/User'; expr_ms: 'user_domain+''/''+user_name';
    expr_my: 'CONCAT(user_domain,''/'',user_name)'),
...
var col_responses: array [0..5] of TCOLINFO =
    ...
    (defwidth: 50; title: 'Класс'; expr_ms: 'user_class'; expr_my: ''),
...
begin
  if TabControl.Visible and StringGrid.Visible then
    for n := 0 to StringGrid.ColCount - 1 do
      WriteConfigInt(false, OURAPPNAME + '\Page' + inttostr(TabControl.TabIndex), 'colwidth' + inttostr(
        n), StringGrid.ColWidths[n]);
end;
...
begin
  ...
  begin  //services
    case idx of
      0:
        Result := FormatDateTime('dd/mm/yy hh:nn', i.GetFieldValueAsDateTime(i.obj, f));
      1:
        Result := GetFieldValueAsDelphiString(i, f);
      2:
        Result := GetFieldValueAsDelphiString(i, f);
      3:
        Result := GetFieldValueAsDelphiString(i, f);
      ...
      5:
        Result := GetFieldValueAsDelphiString(i, f);
      6:
        Result := inttostr(i.GetFieldValueAsInt(i.obj, f));
      7:
        Result := inttostr(i.GetFieldValueAsInt(i.obj, f));
      8:
        Result := inttostr(i.GetFieldValueAsInt(i.obj, f));
      9:
        Result := Format('%1.2f', [i.GetFieldValueAsDouble(i.obj, f)]);
      10:
        Result := GetFieldValueAsDelphiString(i, f);
    end;
  end
  ...
  begin  //services_summ
    case idx of
      0:
        Result := FormatDateTime('dd/mm/yy', DateTimePicker1.DateTime) + ' - ' + FormatDateTime('dd/mm/yy', now());
 //todo: pass now() and DateTimePicker1.DateTime thru parameters to this function
      1:
        Result := GetFieldValueAsDelphiString(i, f);
      2:
        Result := inttostr(i.GetFieldValueAsInt(i.obj, f));
  ...
  begin  //events
    case idx of
      0:
        Result := FormatDateTime('dd/mm/yy hh:nn', i.GetFieldValueAsDateTime(i.obj, f));
      1:
        if i.GetFieldValueAsInt(i.obj, f) > 0 then
          Result := S_IMP else Result := '';
      2:
        Result := GetFieldValueAsDelphiString(i, f);
      3:
        Result := GetFieldValueAsDelphiString(i, f);
  ...
  begin  //responses
    case idx of
      0:
        Result := FormatDateTime('dd/mm/yy', i.GetFieldValueAsDateTime(i.obj, f));
      1:
        Result := GetFieldValueAsDelphiString(i, f);
      2:
        Result := GetFieldValueAsDelphiString(i, f);
...
begin
  ...
  msg_login := RegisterWindowMessage('_KSDBViewLogin');
  msg_foreground := RegisterWindowMessage('_KSDBViewSetForeground');
  ...
  begin
   //insert new services into pricestable
    ...
    begin
      ...
      if cnt <= 0 then
      begin
        if not sql.Exec('INSERT INTO TPrices VALUES(' + inttostr(services[n].id) + ',''' +
          sql.EscapeString(services[n].dbname) + ''',0,0,0,0)') then
        begin
          err := true;
          break;
        end;
...
begin
  ...
  begin
    ...
    WriteStringCRLF(h, '<title>' + TabControl.Tabs[TabControl.TabIndex] + ' [KagamiShell]' + '</title>');
    WriteStringCRLF(h, '<style type="text/css">');
    WriteStringCRLF(h, 'body, td, p {font-family:Segoe UI;font-size:9pt;}');
