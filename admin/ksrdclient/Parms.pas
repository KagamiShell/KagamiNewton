...
const fps_list : array [0..12] of single =
                 (0.2, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 7.0, 8.0, 10.0, 12.0, 30.0, 60.0);
...
begin
  ...
  if reg.OpenKeyReadOnly('Software\Team KagamineP\KSRD') then
    begin
      try
...
begin
  reg := TRegistry.Create;
  if reg.OpenKey('Software\Team KagamineP\KSRD',TRUE) then
    begin
      try
