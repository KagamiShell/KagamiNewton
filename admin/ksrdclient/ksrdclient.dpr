 ...
 if ParamCount=1 then
  begin
   if not RA_Connect(pchar(ParamStr(1))) then
     MessageBox(0,pchar('Не удалось подключиться к удаленному компьютеру '+ParamStr(1)+#13#10'Убедитесь, что на нем установлена ученическая часть программы'),'Ошибка',MB_OK or MB_ICONERROR)
   else
    begin
...
begin
  ...
  Application.Title := 'Удаленное управление версия 1.0.0 от 01.01.2022 00:00';
