  ...
  if ( IsAdminProgram() )
     {
       if ( IsAdminAccount() )
          ...
          {
            if ( !silent )
               MessageBox(parent,"Для запуска приложения требуются права Администратора.",NULL,MB_OK | MB_ICONERROR);
     ...
     {
       if ( !silent )
          MessageBox(parent,"Запуск возможен только из под учительской программы!",NULL,MB_OK | MB_ICONERROR);
