...
// global constants
const KAGAMI_BASE = 'KagamiShell';
const KAGAMI_USER = 'ks_service';
const KAGAMI_BACKUP_USER = 'ks_backupoperator';
const KAGAMI_PWD  = 'cthdbc';
...
begin
 ...
 if not IsBaseExists(obj,KAGAMI_BASE) then
  begin
   if not Query(obj,'CREATE DATABASE '+KAGAMI_BASE) then
    Exit;
   b_needchangeowner:=true;
  end;

 if not Query(obj,'USE '+KAGAMI_BASE) then
  Exit;
 ...
 if not IsObjectExists(obj,'TUserResponses') then
  begin
   ...
   SQL.Add(' user_name   VARCHAR(256),');
   SQL.Add(' user_class    VARCHAR(256),');
 ...
 if not IsObjectExists(obj,'PAddUserResponse') then
  begin
   ...
   SQL.Add('@msg_text    TEXT');
   SQL.Add('AS');
   SQL.Add('INSERT INTO TUserResponses VALUES(@msg_kind,@msg_title,@user_name,@user_class,@msg_text,GETDATE())');
   if not Query(obj,SQL.Text) then
    Exit;
  end;
 ...
 // process internal kagami users
 if not CreateLogin(obj,KAGAMI_BACKUP_USER,KAGAMI_PWD,'') then
  Exit;
 if not AddUserAsBackupOperatorToCurrentBase(obj,KAGAMI_BACKUP_USER) then
  Exit;
 
 if not CreateLogin(obj,KAGAMI_USER,KAGAMI_PWD,KAGAMI_BASE) then
  Exit;
 if not AddUserToCurrentBase(obj,KAGAMI_USER) then
  Exit;

 SQL.Clear();
 SQL.Add('GRANT EXECUTE ON PAddServiceString TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PAddEventString TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PAddUserResponse TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PVipLogin TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PVipLoginByPwd TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PVipRegister TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PVipDelete TO '+KAGAMI_USER);
 SQL.Add('GRANT EXECUTE ON PVipClearPass TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TCompProfiles TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TVarsProfiles TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TContentProfiles TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TCompSettingsRules TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TSettingsRules TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TClientUpdate TO '+KAGAMI_USER);
 SQL.Add('GRANT SELECT ON TClientUpdateNoShell TO '+KAGAMI_USER);
 if not Query(obj,SQL.Text) then
  Exit;
...
begin
 Result:=SQL_Connect(s_server,KAGAMI_USER,KAGAMI_PWD,KAGAMI_BASE);
end;

function SQL_ConnectAsBackupOperator(const s_server:pchar):pointer cdecl;
begin
 Result:=SQL_Connect(s_server,KAGAMI_BACKUP_USER,KAGAMI_PWD,'');
end;

function SQL_ConnectAsNormalUser(const s_server,s_login,s_pwd:pchar):pointer cdecl;
begin
 Result:=SQL_Connect(s_server,s_login,s_pwd,KAGAMI_BASE);
end;
