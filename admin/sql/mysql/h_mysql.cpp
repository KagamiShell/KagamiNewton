...
// global constants
#define KAGAMI_BASE  "KagamiShell"
#define KAGAMI_USER  "ks_service"
#define KAGAMI_BACKUP_USER  "ks_backupoperator"
#define KAGAMI_PWD   "cthdbc"
#define REG_COMMON_KEY "Software\\Team KagamineP\\KagamiShell\\KagamiShellCommon"
                      ...
                      if ( v != -1 )
                         {
                           if ( v < 30000000 )
                              {
                                c->Disconnect();
                                c->SetLastError("Необходимо установить переменную MySQL-сервера \'max_allowed_packet\' как минимум на 32МБ (см. справку по установке KagamiShell)");
                                b_continue = FALSE;
                              }
                         }
                    }
...
////////////////////////////////////////
// KagamiShell-specific functions
////////////////////////////////////////
...
EXPORT BOOL __cdecl SQL_PrepareDB(void *obj)
{
  ...
  if ( obj )
     {
       if ( !Query(obj,"CREATE DATABASE IF NOT EXISTS " KAGAMI_BASE " DEFAULT CHARACTER SET cp1251 COLLATE cp1251_general_ci") )
          return rc;

       if ( !Query(obj,"USE " KAGAMI_BASE) )
          return rc;
       ...
       if ( !IsProcExists(obj,"PAddServiceString",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PAddEventString",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PAddUserResponse",KAGAMI_BASE) )
       {
        const char *q = 
        ...
        "_user_name   VARCHAR(256),\n"
        "_user_class    VARCHAR(256),\n"
        "_msg_text    TEXT)\n"
        "BEGIN\n"
        "INSERT INTO TUserResponses VALUES(_msg_kind,_msg_title,_user_name,_user_class,_msg_text,NOW());\n"
        "END;\n";
        if ( !Query(obj,q) )
           return rc;
       }
       ...
       if ( !IsProcExists(obj,"PVipLogin",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PVipLoginByPwd",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PVipRegister",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PVipDelete",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PVipClearPass",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PCompProfilesAddModify",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PVarsProfilesAddModify",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PContentProfilesAddModify",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       if ( !IsProcExists(obj,"PClientUpdateAdd",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       //create client-update stored proc for Add
       if ( !IsProcExists(obj,"PClientUpdateNoShellAdd",KAGAMI_BASE) )
       {
        const char *q = 
       ...
       // process internal kagami user
       if ( !SQL_CreateUser(obj,KAGAMI_USER,KAGAMI_PWD) )
          return rc;

       rc = TRUE;
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PAddServiceString",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PAddEventString",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PAddUserResponse",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PVipLogin",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PVipLoginByPwd",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PVipRegister",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PVipDelete",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"EXECUTE","PROCEDURE PVipClearPass",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TCompProfiles",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TVarsProfiles",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TContentProfiles",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TCompSettingsRules",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TSettingsRules",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TClientUpdate",KAGAMI_USER);
       rc = rc && GrantInternal(obj,"SELECT","TClientUpdateNoShell",KAGAMI_USER);
     }
...
{
  return SQL_Connect(s_server,KAGAMI_USER,KAGAMI_PWD,KAGAMI_BASE);
}
...
{
  return SQL_Connect(s_server,s_login,s_pwd,KAGAMI_BASE);
}
