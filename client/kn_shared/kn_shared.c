...
#define REGPATH		"Software\\KagamineP\\KagamiShell"
...
BOOL CALLBACK MessageBoxDialog(HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam)
{
  ...
  if ( message == WM_INITDIALOG )
     {
       ...
       SetProp(hwnd,"_KNMessageBoxProp",(HANDLE)1);
  ...
  if ( message == WM_CLOSE || (message == WM_COMMAND && LOWORD(wParam) == IDCANCEL) )
     {
       RemoveProp(hwnd,"_KNMessageBoxProp");
       EndDialog(hwnd,-1);
     }
...
__declspec(dllexport) int __cdecl KNMessageBox(HWND hwnd, LPCSTR lpText, LPCSTR lpCaption, LPCSTR lpList, UINT uDefaultListItem, UINT uType)
{
  SMessageBox p;
  p.caption = lpCaption;
  p.text = lpText;
  ...
  if ( count )
     {
       if ( count == 1 || !hwnd )
          ...
          {
            int rc = KNMessageBox(hwnd,S_DISKSELECT2,S_DISKSELECT1,fnames,0,KNICON_SAVE);
...
__declspec(dllexport) char* __cdecl GetFavoritesPath(char *out)
{
  ...
  w = FindWindow("_NewtonClass",NULL);
  if ( w )
     {
       ...
       if ( p[0] )
          {
            ...
            if ( atom )
               {
                 HWND w = FindWindow("_NewtonClass",NULL);
...
__declspec(dllexport) BOOL __cdecl IsStudentFolderAllowed(void)
{
  return ReadRegDword(HKCU,REGPATH,"student_in_menu",1) != 0;
}


__declspec(dllexport) char* __cdecl GetStudentFolderPath(char *out)
{
  HWND w;
...
__declspec(dllexport) char* __cdecl GetStudentFolderName(char *s)
{
  HWND w;
  
  lstrcpy(s,S_STUDENTFOLDERNAME);
...
__declspec(dllexport) int __cdecl KNOpenSaveDialog(HWND hwnd,const char *s_text,const char *s_caption,char *out,char *out_name)
{
  int rc = KNOPENSAVE_CANCEL;
  char s[2048],*p;
  char buff[MAX_PATH];
  int last_save_to;
  int iflash=-1,idiskette=-1,icdrom=-1,istudentfolder=-1,iuserfolder=-1,iexfolder=-1;
  int count, idx;
  ...
  if ( IsStudentFolderAllowed() )
     {
       lstrcpy(p,GetStudentFolderName(buff));
       p += lstrlen(p) + 1;
       istudentfolder = count;
       count++;
     }
  ...
  last_save_to = ReadRegDword(HKCU,REGPATH,"KNLastSaveTo",0);
  idx = KNMessageBox(hwnd,s_text,s_caption,s,last_save_to,KNICON_SAVE);
  ...
  WriteRegDword(HKCU,REGPATH,"KNLastSaveTo",idx);
  ...
  if ( idx == istudentfolder )
     {
       GetStudentFolderPath(out);
       rc = KNOPENSAVE_STUDENTFOLDER;
       if ( out_name )
          GetStudentFolderName(out_name);
     }
  else
  if ( idx == iuserfolder )
     {
       GetUserFolderPath(out,0);
       rc = KNOPENSAVE_USERFOLDER;
       if ( out_name )
  ...
  if ( !out[0] )
     {
       if ( rc != KNOPENSAVE_STUDENTFOLDER )
          {
            MessageBox(hwnd,S_RESEMPTY,LS(LS_INFORMATION),MB_OK | MB_ICONWARNING);
          }
       else
          {
            MessageBox(hwnd,S_RESSTUDENTEMPTY,LS(LS_INFORMATION),MB_OK | MB_ICONINFORMATION);
          }
     }
...
__declspec(dllexport) BOOL __cdecl CheckKNVersion(int version)
{
  char s[MAX_PATH];
  
  HWND r_w = FindWindow("_NewtonClass",NULL);
  if ( r_w )
     {
  ...
  wsprintf(s,"Требуется KagamiShell версии v%d.%02d+",version/100,version%100);
  MessageBox(NULL,s,"Ошибка",MB_OK | MB_ICONERROR);
