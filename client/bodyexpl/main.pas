  ...
  while (wnd <> 0) and (wnd <> GetDesktopWindow()) and (wnd <> Handle) do
    begin
      if (GetClassName(wnd, @s, sizeof(s)) > 0) then
        if (s = 'Winamp v1.x') or (s = 'TBodyExplForm') or (s = 'MediaPlayerClassicW') or (s = 'MediaPlayerClassicA') or (s = 'MediaPlayerClassic') or (s = 'TBodyBurnForm') or (s = 'TBodyMailForm') or (s = '_KSSheetWindowClass') or (s = 'TSaverForm') then
          exit;
...
// except end;
 ...
 msg_langchanged:=RegisterWindowMessage('_KPLanguageChanged');
 ...
 count:=0;
 reg:=TRegistry.Create;
 if reg.OpenKeyReadOnly('Software\Team KagamineP\KagamiShell\menu_ext') then
  begin
 ...
 count:=0;
 reg:=TRegistry.Create;
 if reg.OpenKeyReadOnly('Software\Team KagamineP\KagamiShell\menu_ext_rev') then
  begin
   for n:=1 to MAXWINITEMS do
    begin
     state:=false;
...
begin
  reg:=TRegistry.Create;
  if reg.OpenKey('Software\Team KagamineP\KagamiShell', true) then
    begin
      try
