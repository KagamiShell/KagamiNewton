...
{$R *.dfm}
{$INCLUDE ..\ks_shared\KS_Shared.inc}
...
begin
  ...
  begin
    ...
    begin
      ...
      if Msg.message = vip_end_message then
      begin
        ComboBox.ItemsEx.Items[IVipFolder].Caption := GetVIPFolderName(@buf);
        ListView.Items[IVipFolder - 1].Caption := GetVIPFolderName(@buf);
...
begin
  ...
  begin
    if (GetClassName(wnd, @s, sizeof(s)) > 0) then
      if (s = 'Winamp v1.x') or (s = 'TBodyExplForm') or (s = 'MediaPlayerClassicW') or
        (s = 'MediaPlayerClassicA') or (s = 'MediaPlayerClassic') or (s = 'TBodyBurnForm') or
        (s = 'TBodyMailForm') or (s = '_KSSheetWindowClass') or (s = 'TSaverForm') then
        exit;
...
begin
  ...
  Caption := Title;
  hIcon := LoadImage(HInstance, PChar(ComboBox.ItemsEx.Items[ComboBox.ItemIndex].ImageIndex + 100),
...
begin
  ...
  if (Pos(SDiskette, s) = 1) and (IDiskette >= 0) then
  begin
  ...
  if (Pos(SCdrom, s) = 1) and (ICdrom >= 0) then
  begin
  ...
  if (Pos(SVipFolder, s) = 1) and (IVipFolder >= 0) then
  begin
  ...
  if (Pos(SUserFolder, s) = 1) and (IUserFolder >= 0) then
  begin
    try
  ...
  if (Pos(SExFolder, s) = 1) and (IExFolder >= 0) then
  begin
    try
...
begin
  ...
  begin
    ...
    case a of
      ACC_COPY:
        Result := not AnsiContainsText(s, 'C');
      ACC_DELETE:
        Result := not AnsiContainsText(s, 'D');
      ACC_WRITE:
        Result := not AnsiContainsText(s, 'W');
    end;
...
begin
  ...
  begin
    ...
    if UpperCase(Copy(s, Length(s) - 2, 3)) = '.7Z' then
...
begin
  ...
  MenuFreeSpace.Enabled := (IExploringPlace = IDiskette) or (IExploringPlace = IFlash) or (IExploringPlace = ICdrom);
 // свободное место
...
begin
  ...
  begin
    ...
    begin
      ...
      name := LS(1403);
      filename := IncludeTrailingPathDelimiter(ShellListView.Root{ShellListView.RootFolder.PathName}) +
        UniqueArchiveName(ShellListView.Root, name, ext);
...
begin
  ...
  for i := 0 to ShellListView.Items.Count - 1 do
    if ShellListView.Items[i].Selected then
    begin
      if ShellListView.Folders[i].IsFolder or
        (not GDIP_IsSupportedFormat(pchar(ShellListView.Folders[i].PathName))) then
      begin
        Result := false;
        break;
      end;
...
begin
  ...
  for i := 0 to ShellListView.Items.Count - 1 do
    if ShellListView.Items[i].Selected then
      if (not ShellListView.Folders[i].IsFolder) and GDIP_IsSupportedFormat(
        pchar(ShellListView.Folders[i].PathName)) then
      begin
  ...
  begin
    ...
    case action of
      IMGA_ROTATECW:
        GDIP_RotateImage(false, pchar(src), pchar(dest));
      IMGA_ROTATECCW:
        GDIP_RotateImage(true, pchar(src), pchar(dest));
      IMGA_RESIZESMALL:
        GDIP_ReduceImage(640, 480, pchar(src), pchar(dest));
      IMGA_RESIZENORMAL:
        GDIP_ReduceImage(800, 600, pchar(src), pchar(dest));
      IMGA_RESIZEBIG:
        GDIP_ReduceImage(1024, 768, pchar(src), pchar(dest));
      IMGA_CONVERT:
        GDIP_ConvertFile(pchar(src), pchar(dest));
    end;
