...
#define REGPATH		"Software\\Team KagamineP\\KagamiShell"
...
__declspec(dllexport) BOOL __cdecl CreateNewEmptyPy(const char *filename)
{
  return WriteResourceToFile(filename,ID_NEWPY);
}
...
__declspec(dllexport) int __cdecl KSOpenSaveDialog(HWND hwnd,const char *s_text,const char *s_caption,char *out,char *out_name)
{
  ...
  char buff[MAX_PATH];
  int last_save_to;
  int iflash=-1,idiskette=-1,icdrom=-1,iuserfolder=-1,iexfolder=-1;
  int count, idx;
...
__declspec(dllexport) BOOL __cdecl CheckKSVersion(int version)
{
  ...
  wsprintf(s,"KagamiShell v%d.%02d+ is required",version/100,version%100);
