...
void ReadGlobalConfig()
{
  ...
  if ( !computer_loc[0] )
     lstrcpy(computer_loc,"Organization");
  ReadRegStr(HKLM,REGPATH,"computer_desc",computer_desc,"");
  if ( !computer_desc[0] )
     MyGetComputerName(computer_desc);
...
void ChecksAtStartup(void)
{
  ...
  if ( FindCmdLineParm("-turnon") )
     ...
     {
       CreateMutex(NULL,FALSE,"_KagamiShellMutex");
