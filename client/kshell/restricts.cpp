...
static void CopyHookDisable(BOOL state,void *p1,void *p2,void *p3)
{
  if ( state )
     {
       WriteRegStr(HKCU,REG_CLASSES "Directory\\Shellex\\CopyHookHandlers\\KagamiCopyHook",NULL,"{CE2606AC-0A84-4272-97B0-04FF67BA05B6}");
  ...
  else
     {
       ...
       DeleteRegKey(HKCU,REG_CLASSES "Directory\\Shellex\\CopyHookHandlers\\KagamiCopyHook");
     }
}
...
static BOOL IsKagamiWrapperWindow(HWND hwnd)
{
  return (g_opensavedlgs.find(hwnd) != g_opensavedlgs.end()) || GetProp(hwnd,"_KagamiOpenSaveDialogBox") || GetProp(hwnd,"_RPMessageBoxProp");
}
...
static void CheckDialogBoxes(void)
{
  ...
  if ( !SetProp(hwnd,"_KagamiOpenSaveDialogBox",(HANDLE)select) && GetLastError() == ERROR_ACCESS_DENIED )
     {
       ...
       if ( owner && IsMainTorrentWindow(owner,TRUE/*FALSE*/) )
          {
            ...
            if ( StrStrI(s_win,"Preferences") || StrStrI(s_win,"Настройки") ||
                 StrStrI(s_win,"Torrent Properties") || StrStrI(s_win,"Свойства торрента") ||
                 StrStrI(s_win,"RSS Downloader") || StrStrI(s_win,"RSS загрузчик") || StrStrI(s_win,"Диспетчер RSS-рассылок") ||
                 StrStrI(s_win,"Add RSS Feed") || StrStrI(s_win,"Добавить RSS-рассылку") ||
                 StrStrI(s_win,"Enter logfile") || StrStrI(s_win,"Ведение файла журнала") ||
                 !lstrcmpi(s_win,"Create New Torrent") || !lstrcmpi(s_win,"Создать новый торрент"))
               {
                 PostMessage(hwnd,WM_CLOSE,0,0);
               }
            else
