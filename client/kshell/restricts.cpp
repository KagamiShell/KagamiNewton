...
static void CopyHookDisable(BOOL state,void *p1,void *p2,void *p3)
{
  if ( state )
     {
       WriteRegStr(HKCU,REG_CLASSES "Directory\\Shellex\\CopyHookHandlers\\KagamiShellCopyHook",NULL,"{CE2606AC-0A84-4272-97B0-04FF67BA05B6}");
  ...
  else
     {
       ...
       DeleteRegKey(HKCU,REG_CLASSES "Directory\\Shellex\\CopyHookHandlers\\KagamiShellCopyHook");
     }
}
...
static BOOL IsKagamiShellWrapperWindow(HWND hwnd)
{
  return (g_opensavedlgs.find(hwnd) != g_opensavedlgs.end()) || GetProp(hwnd,"_KagamiShellOpenSaveDialogBox") || GetProp(hwnd,"_KNMessageBoxProp");
}
  ...
  if ( !lstrcmpi(s_class,"Winamp v1.x") )
     {
       ...
       if ( lstrcmpi(s_win,"Jump to file") )
          {
            if ( !IsKagamiShellWrapperWindow(hwnd) )
               {
...
static void DialogBoxReqProcessing(HWND hwnd,HWND select,BOOL mso,BOOL icq,BOOL corel)
{
  ...
  rc = KNOpenSaveDialog(hwnd,LS(3067),LS(3068),dest_path,sel_name);
  
  if ( rc == KNOPENSAVE_CANCEL )
     {
       PostMessage(hwnd,WM_CLOSE,0,0);
       return;
     }

  is_user_folder = (rc == KNOPENSAVE_USERFOLDER);
...
static void CheckDialogBoxes(void)
{
  ...
  if ( !SetProp(hwnd,"_KagamiShellOpenSaveDialogBox",(HANDLE)select) && GetLastError() == ERROR_ACCESS_DENIED )
     {
                      ...
                      if ( w_combo && w_edit && w_select && w_adv && w_ok )
                         {
                           ...
                           int rc = KNOpenSaveDialog(hwnd,LS(3211),LS(3210),dest_path,sel_name);

                           if ( IsWindow(hwnd) )
                              {
                                if ( rc == KNOPENSAVE_CANCEL || IsStrEmpty(dest_path) )
                                   {
                                     PostMessage(hwnd,WM_CLOSE,0,0);
                                   }
                                else
