       ...
       if ( state )
          {
            ...
            if ( can_set )
               {
                 char dest[MAX_PATH] = "";
                 GetFileNameInTrueSystem32Dir(KS_GINA,dest);

                 if ( IsFileExist(dest) )
                    {
                      WriteRegStr(HKLM,winlogon_key,"KSSaveGinaDLL",old_gina);
                      WriteRegStr(HKLM,winlogon_key,"GinaDLL",KS_GINA);
                    }
               }
          }
       else
          {
            BOOL can_remove = !lstrcmpi(PathFindFileName(old_gina),KS_GINA);

            if ( can_remove )
               {
                 char saved_gina[MAX_PATH];
                 ReadRegStr(HKLM,winlogon_key,"KSSaveGinaDLL",saved_gina,"");
            ...
            DeleteRegValue(HKLM,winlogon_key,"KSSaveGinaDLL");
          }
     }
}
...
static void InstallUninstallOurClassesInternal(BOOL b_inst)
{
  ...
  // CopyHook
  GetFileNameInLocalAppDir("ksexhook.dll",s);
  if ( b_inst )
     ActiveXRegister32("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}","Kagami CopyHook",s,"Apartment",NULL);
  else
     ActiveXUnregister32("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}");

  if ( IsWOW64() )
     {
       GetFileNameInLocalAppDir("ksexhook64.dll",s);
       if ( b_inst )
          ActiveXRegister64("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}","Kagami CopyHook",s,"Apartment",NULL);
       else
  ...
  GetFileNameInLocalAppDir("ksexhook.dll",s);
  if ( b_inst )
     ActiveXRegister32("{9BE88898-18E7-4528-8886-6358AAC2A61A}","Kagami ShellExecuteHook",s,"Apartment",NULL);
  else
  ...
  if ( IsWOW64() )
     {
       GetFileNameInLocalAppDir("ksexhook64.dll",s);
       if ( b_inst )
          ActiveXRegister64("{9BE88898-18E7-4528-8886-6358AAC2A61A}","Kagami ShellExecuteHook",s,"Apartment",NULL);
       else
