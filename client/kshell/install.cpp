       ...
       if ( state )
          {
            ...
            if ( can_set )
               {
                 char dest[MAX_PATH] = "";
                 GetFileNameInTrueSystem32Dir(KP_GINA,dest);

                 if ( IsFileExist(dest) )
                    {
                      WriteRegStr(HKLM,winlogon_key,"KPSaveGinaDLL",old_gina);
                      WriteRegStr(HKLM,winlogon_key,"GinaDLL",KP_GINA);
                    }
               }
          }
       else
          {
            BOOL can_remove = !lstrcmpi(PathFindFileName(old_gina),KP_GINA);

            if ( can_remove )
               {
                 char saved_gina[MAX_PATH];
                 ReadRegStr(HKLM,winlogon_key,"KPSaveGinaDLL",saved_gina,"");
            ...
            DeleteRegValue(HKLM,winlogon_key,"KPSaveGinaDLL");
          }
     }
}
