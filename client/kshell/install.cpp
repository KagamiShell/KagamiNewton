       ...
       if ( state )
          {
            ...
            if ( can_set )
               {
                 char dest[MAX_PATH] = "";
                 GetFileNameInTrueSystem32Dir(KN_GINA,dest);

                 if ( IsFileExist(dest) )
                    {
                      WriteRegStr(HKLM,winlogon_key,"KNSaveGinaDLL",old_gina);
                      WriteRegStr(HKLM,winlogon_key,"GinaDLL",KN_GINA);
                    }
               }
          }
       else
          {
            BOOL can_remove = !lstrcmpi(PathFindFileName(old_gina),KN_GINA);

            if ( can_remove )
               {
                 char saved_gina[MAX_PATH];
                 ReadRegStr(HKLM,winlogon_key,"KNSaveGinaDLL",saved_gina,"");
            ...
            DeleteRegValue(HKLM,winlogon_key,"KNSaveGinaDLL");
          }
     }
}
...
static void InstallUninstallOurClassesInternal(BOOL b_inst)
{
  ...
  // KN_API
  GetFileNameInTrueSystem32Dir("kn_api.dll",s);
  if ( b_inst )
  ...
  // CopyHook
  GetFileNameInLocalAppDir("knexhook.dll",s);
  if ( b_inst )
     ActiveXRegister32("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}","KagamiShell CopyHook",s,"Apartment",NULL);
  else
     ActiveXUnregister32("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}");

  if ( IsWOW64() )
     {
       GetFileNameInLocalAppDir("knexhook64.dll",s);
       if ( b_inst )
          ActiveXRegister64("{CE2606AC-0A84-4272-97B0-04FF67BA05B6}","KagamiShell CopyHook",s,"Apartment",NULL);
       else
  ...
  GetFileNameInLocalAppDir("knexhook.dll",s);
  if ( b_inst )
     ActiveXRegister32("{9BE88898-18E7-4528-8886-6358AAC2A61A}","KagamiShell ShellExecuteHook",s,"Apartment",NULL);
  else
  ...
  if ( IsWOW64() )
     {
       GetFileNameInLocalAppDir("knexhook64.dll",s);
       if ( b_inst )
          ActiveXRegister64("{9BE88898-18E7-4528-8886-6358AAC2A61A}","KagamiShell ShellExecuteHook",s,"Apartment",NULL);
       else
...
// remove some left from old versions of KagamiShell 
