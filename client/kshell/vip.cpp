                 ...
                 if ( mbsize > vip_folder_limit )
                    {
                      ...
                      if ( !use_post_msg )
                         ...
                         {
                           PostMessage(GetMainWnd(),KS_MESSAGEBOX,(WPARAM)GlobalAddAtom(s),0);
                         }
                    }
               }
          }
  ...
  if ( vip_session[0] )
     {
       ...
       PostMessage(HWND_BROADCAST,RegisterWindowMessage("_KSVIPSessionEnd"),0,0);
  ...
  if ( login && login[0] && folder && folder[0] )
     {
       ...
       PostMessage(HWND_BROADCAST,RegisterWindowMessage("_KSVIPSessionBegin"),0,0);
       SendDynamicInfoToServer();
       ...
       if ( !IsStrEmpty(device_str) )
          {
            if ( IsWindowVisible(GetDlgItem(hwnd,IDC_EDIT3)) )
               ...
               { //login
                 ...
                 PostMessage(GetMainWnd(),KS_LOGINVIPSESSION_NOFORCE,0,(LPARAM)GlobalAddAtom(device_str));
               }
          }
