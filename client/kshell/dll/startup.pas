...
function TKSStartupForm.IsCyr: boolean;
begin
  Result := (GetACP() = 1251);
end;


function TKSStartupForm.FromMachineType2Idx(machine_type: integer): integer;
begin
  Result := 0;
  case machine_type of
    MACHINE_TYPE_STUDENT:
      Result := 0;
    MACHINE_TYPE_TEACHER:
      Result := 1;
    MACHINE_TYPE_OPERATOR:
      Result := 2;
    MACHINE_TYPE_ORGANIZATION:
      Result := 3;
...
begin
  ...
  case idx of
    0:
      Result := MACHINE_TYPE_STUDENT;
    1:
      Result := MACHINE_TYPE_TEACHER;
    2:
      Result := MACHINE_TYPE_OPERATOR;
    3:
      Result := MACHINE_TYPE_ORGANIZATION;
    4:
      Result := MACHINE_TYPE_TERMINAL;
    5:
      Result := MACHINE_TYPE_HOME;
    6:
      Result := MACHINE_TYPE_OTHER;
  end;
end;

constructor TKSStartupForm.CreateForm(p: pinteger);
var n: integer;
begin
  ...
  if IsCyr then
  begin
    Caption := 'Мастер настройки клиентской части KagamiShell';
    ...
    ComboBox1.Items.Add('Компьютер в организации/офисе');
  ...
  begin
    Caption := 'Settings master for KagamiShell Client';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if MessageBoxW(Handle, pwidechar(widestring(
          'Адрес сервера не введен. Продолжить?')), pwidechar(
          widestring('Вопрос')), MB_YESNO or MB_DEFBUTTON2 or MB_ICONQUESTION) <> IDYES then
        begin
          Edit3.SetFocus;
          exit;
        end;
...
begin
  ...
  begin
    if MessageBoxW(Handle, pwidechar(widestring('Прервать работу мастера?')),
      pwidechar(widestring('Вопрос')), MB_YESNO or MB_DEFBUTTON2 or MB_ICONQUESTION) <> IDYES then
      CanClose := false;
  end;
end;

procedure TKSStartupForm.TabSheet3Show(Sender: TObject);
begin
  Edit3.SetFocus;
end;

procedure TKSStartupForm.ComboBox2Change(Sender: TObject);
begin
  if not IsCyr then
  begin
    if visible then
...
procedure TKSStartupForm.TabSheet7Show(Sender: TObject);
begin
// Edit4.SetFocus;
end;

procedure TKSStartupForm.TabSheet2Show(Sender: TObject);
begin
  ComboBox1.SetFocus;
end;
