...
BOOL RemoteUninstallInternal(CServiceManager *sm,BOOL b_restart,BOOL b_force)
{
  ...
  if ( StrStrI(PathFindFileName(our_exe),"tmp") )
     {
       // schedule deletion of this .exe and kn_shared.dll
       MoveFileEx(our_exe,NULL,MOVEFILE_DELAY_UNTIL_REBOOT);
       char t[MAX_PATH] = "";
       GetFileNameInLocalAppDir("kn_shared.dll",t);
       MoveFileEx(t,NULL,MOVEFILE_DELAY_UNTIL_REBOOT);

       // get original install path
       char inst_path[MAX_PATH] = "";
       ReadRegStr(HKLM,"SOFTWARE\\KagamineP\\KagamiShell","Install_Dir",inst_path,"");
            ...
            if ( sm->HighLevelUninstall() )
               {
                 ...
                 DeleteRegValue(HKLM,"SOFTWARE\\KagamiShell","Install_Dir");
                 DeleteRegValue(HKLM,"SOFTWARE\\KagamiShell","update_finish_flag");

                 // delete all files in original install directory
                 char target_exe[MAX_PATH];
                 lstrcpy(target_exe,inst_path);
                 PathAppend(target_exe,"kshell_svc.exe");
...
BOOL StartupMasterDialog()
{
  ...
  HINSTANCE lib = LoadLibrary("kshell.dll");
  ...
  if ( ShowStartupMasterDialog )
     {
       ...
       ReadGlobalConfig(i.computer_type,i.computer_loc,i.computer_desc,i.server_ip);

       if ( ShowStartupMasterDialog(&i) )
          {
            WriteGlobalConfig(i.computer_type,i.computer_loc,i.computer_desc,i.server_ip);
            SaveRollback1();
          }
     }
  ...
  if ( SearchParam("-setup") )
     {
       ...
       if ( IsAdminAccount() )
          {
            printf("KagamiShell client installer\n");  // for decoration
