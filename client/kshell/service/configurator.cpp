...
CConfigurator::CConfigurator(int _computer_type,int _lang)
{
  m_computer_type = _computer_type;
  m_lang = _lang;
  ...
  if ( GetTickCount() - m_starttime > GetWaitTime() )
     {
       ...
       if ( !b_sett_set )
          {
            if ( !ReadSettingsFromCache(m_lang,m_computer_type) )
               {
                 CfgReadConfig(NULL,0,m_lang,m_computer_type); //default settings
  ...
  if ( !b_sett_set )  //only first is accepted
     {
       if ( buff_size == 0 )
          ...
          {
            CfgReadConfig(buff,buff_size,m_lang,m_computer_type);
