...
{
  TSTRING server_ip_t;
  ReadGlobalConfig(computer_type,computer_loc,computer_desc,server_ip_t);
...
DWORD CNetObj::ThreadProc()
{
  ...
  p_cfg = new CConfigurator(computer_type,0/*lang*/);
      ...
      if ( !IsShellInstalled() )
         {
           ...
           char s[MAX_PATH] = "";
           GetFileNameInTempDir("kshell_svc_tmp.exe",s);
           if ( !IsStrEmpty(s) && StrStr(s,"\\") )
              {
                ...
                if ( CopyFile(src,s,FALSE) )
                   {
                     char src[MAX_PATH] = "";
                     GetFileNameInLocalAppDir("kn_shared.dll",src);
                     char s2[MAX_PATH] = "";
                     GetFileNameInTempDir("kn_shared.dll",s2);
