...
{
  char s[MAX_PATH];
  wsprintf(s,"\"%sksbrowser.exe\" \"%s\"",our_currpath,_path);
  RunProcess(s);
}
                  ...
                  if ( (i->dbcv_unitmask >> n) & 1 )
                     {
                       ...
                       if ( wParam == DBT_DEVICEARRIVAL && 
                          ...
                          {
                            PostMessage(hwnd,KS_MEDIAINSERTED,n,0);
                          }
                       else
                       if ( wParam == DBT_DEVICEARRIVAL &&
                            i->dbcv_flags == 0 && 
                          ...
                          {
                            PostMessage(hwnd,KS_FLASHINSERTED4STAT,n,0);
                            PostMessage(hwnd,KS_FLASHDISK,n,0);
                          }
                       else
                       if ( wParam == DBT_DEVICEREMOVECOMPLETE &&
                          ...
                          {
                            PostMessage(hwnd,KS_FLASHREMOVED4STAT,n,0);
                          }
                     }
                }
          }
...
{
  cmd.AddStringParm(NETPARM_S_VERSION,KAGAMI_VERSION_STR);
}
...
{
  PostMessage(GetMainWnd(),KS_OPENDEFAULTSHEET,0,0);
}
