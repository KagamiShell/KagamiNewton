...
const char* ExpandStatusText(const char *s)
{
  ...
  StrReplaceI(out,"%COMPUTER%",computer_desc);
  StrReplaceI(out,"%KN_COMPUTER%",computer_desc);
  StrReplaceI(out,"%KN_LOC%",computer_loc);
  StrReplaceI(out,"%LOC%",computer_loc);
  StrReplaceI(out,"%STUDENT%",student_session);
  StrReplaceI(out,"%KN_STUDENTSESSION%",student_session);
...
void M_Monitoroff(void)
{
  ...
  rc = KNMessageBox(GetMainWnd(),LS(3005),LS(3006),list,0,(UINT)icon);
...
{
  char msg[MAX_PATH];
  wsprintf(msg,STR_001,computer_loc,computer_desc);
                  ...
                  if ( (i->dbcv_unitmask >> n) & 1 )
                     {
                       ...
                       if ( wParam == DBT_DEVICEARRIVAL && 
                          ...
                          {
                            PostMessage(hwnd,KN_MEDIAINSERTED,n,0);
                          }
                       else
                       if ( wParam == DBT_DEVICEARRIVAL &&
                            i->dbcv_flags == 0 && 
                          ...
                          {
                            PostMessage(hwnd,KN_FLASHINSERTED4STAT,n,0);
                            PostMessage(hwnd,KN_FLASHDISK,n,0);
                          }
                       else
                       if ( wParam == DBT_DEVICEREMOVECOMPLETE &&
                          ...
                          {
                            PostMessage(hwnd,KN_FLASHREMOVED4STAT,n,0);
                          }
                     }
                }
          }
...
{
  cmd.AddStringParm(NETPARM_S_VERSION,NEWTON_VERSION_STR);
}
...
{
  PostMessage(GetMainWnd(),KN_OPENDEFAULTSHEET,0,0);
}
