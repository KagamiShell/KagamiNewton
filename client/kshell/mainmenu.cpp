...
{
  {10, 3053, M_Studentsession, IDM_STUDENTBEGIN     },
  {9,  3054, M_Studentsession, IDM_STUDENTEND       },
...
{
  PostMessage(GetMainWnd(),KN_EXECMAINMENUITEM,m_idx,0);
}
...
void CODItemShortcut::Execute()
{
  if ( b_allow_execute )
     {
       PostMessage(GetMainWnd(),KN_RUNPROGRAMINTERNAL,(WPARAM)m_shortcut,(LPARAM)m_sheet);
     }
  else
     {
       PostMessage(GetMainWnd(),KN_MESSAGE,(WPARAM)(LS(3052)),0);
     }
}
...
{
  PostMessage(GetMainWnd(),KN_EXECQLITEM,(WPARAM)m_item,0);
}
  ...
  : CPopupMenu(NULL) //allocates popup menu
{
  ...
  if ( student_in_menu )
     AddMMItem(student_session[0]?IDM_STUDENTEND:IDM_STUDENTBEGIN);
  if ( gc_info_in_menu )
  ...
  if ( logoff_in_menu || reboot_in_menu || shutdown_in_menu || (student_in_menu && student_session[0]) )
     {
       ...
       if ( student_in_menu && student_session[0] )
          {
            if ( logoff_in_menu || reboot_in_menu || shutdown_in_menu )
               AddMMSeparator(m2);
            AddMMItem(IDM_STUDENTEND,m2);
          }
