...
LRESULT CALLBACK ShellProc(int code,WPARAM wParam,LPARAM lParam)
{
  ...
  hwnd = FindWindow("_NewtonClass",NULL);
  ...
  switch ( code )
  {
    ...
    case HSHELL_LANGUAGE:
         PostMessage(hwnd,KN_LANGUAGE,wParam,lParam);
         break;
  };
       ...
       if ( wParam == VK_LWIN || wParam == VK_RWIN )
          {
            if ( !wkey_ignore )
               {
                 PostMessage(FindWindow("_NewtonClass",NULL),KN_WINKEY,0,0);
               }
          }
...
static HWND IsMSOProtectedDlgWindow(HWND hwnd)
{
  ...
  return GetProp(hwnd,"_NewtonOpenSaveDialogBox");
}
...
LRESULT CALLBACK MsgFilterProc(int code,WPARAM wParam,LPARAM lParam)
{
  ...
  switch ( code )
  {
    case MSGF_DIALOGBOX:
                        ...
                        if ( !IsWindowVisible(root) || !GetProp(root,"_NewtonOpenSaveDialogBox") )
                           break;
