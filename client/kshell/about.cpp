...
static const char *szClassName = "_KNAboutClass";
  ...
  if ( s_progress[0] || s_lic_name[0] )
     {
       ...
       if ( !g_b_warn )
          {
            if ( s_lic_name[0] )
               wsprintfW(s,L"%s\n%s: %d\n\n",CUnicodeRus(s_lic_name).Text(),CUnicode(LS(3000)).Text(),i_lic_computers);
            else
               lstrcpyW(s,L"\n\n\n");
          }
...
void AboutBoxUpdateLicInfo(const char *lic_name,int lic_computers)
{
  if ( thread )
     {
       HWND w = FindWindow(szClassName,NULL);
