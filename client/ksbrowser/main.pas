...
type
  TKSFolderBrowserForm = class(TForm)
...
var
  KSFolderBrowserForm: TKSFolderBrowserForm;
...
{$R *.dfm}
{$INCLUDE ..\ks_shared\KP_Shared.inc}

var
   g_SortColumn : integer = -1;


procedure TKSFolderBrowserForm.DropperCheckTargetWindow(var Allow: Boolean);
var
  wnd, old_wnd : HWND;
  p   : TPoint;
...
begin
  ...
  if reg.OpenKeyReadOnly('Software\Team KagamineP\KagamiShell') then
    begin
      try
...
function TKSFolderBrowserForm.IsWinampFile(PathName: string): boolean;
var s,ext:string;
    reg:TRegistry;
begin
 Result:=false;
...
function TKSFolderBrowserForm.GetWinampPath:string;
var reg:TRegistry;
    s:string;
begin
...
procedure TKSFolderBrowserForm.RunBodyToolWithParms(tool,parms:string);
...
procedure TKSFolderBrowserForm.FormCreate(Sender: TObject);
var w : HWND;
    atom : integer;
...
procedure TKSFolderBrowserForm.FormDestroy(Sender: TObject);
begin
 Dropper.Free;
end;


procedure TKSFolderBrowserForm.FormShow(Sender: TObject);
begin
 Panel1.SetFocus;
end;


procedure TKSFolderBrowserForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
 if key=#27 then
  Close;
end;
...
procedure TKSFolderBrowserForm.FormConstrainedResize(Sender: TObject; var MinWidth,
  MinHeight, MaxWidth, MaxHeight: Integer);
begin
 MinWidth:=300;
 MinHeight:=200;
...
procedure TKSFolderBrowserForm.ShellTreeViewEditing(Sender: TObject; Node: TTreeNode;
  var AllowEdit: Boolean);
begin
 AllowEdit:=false;
end;
...
procedure TKSFolderBrowserForm.ShellListViewEditing(Sender: TObject; Item: TListItem;
  var AllowEdit: Boolean);
begin
 AllowEdit:=false;
end;
...
procedure TKSFolderBrowserForm.ShellListViewMouseDown(Sender: TObject;
...
procedure TKSFolderBrowserForm.ShellListViewMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TKSFolderBrowserForm.ShellListViewContextPopup(Sender: TObject;
...
procedure TKSFolderBrowserForm.DropperDropUp(var Files: TStringList);
var i:integer;
begin
  if ShellListView.SelectedFolder<>nil then
   begin
...
procedure TKSFolderBrowserForm.ShellListViewKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TKSFolderBrowserForm.ShellListViewDblClick(Sender: TObject);
var p:pchar;
    f,w:HWND;
...
procedure TKSFolderBrowserForm.ShellListViewSortBy(idx:integer);
begin
  g_SortColumn := idx;
...
procedure TKSFolderBrowserForm.ShellListViewColumnClick(Sender: TObject;
  Column: TListColumn);
begin
...
procedure TKSFolderBrowserForm.tbWinampClick(Sender: TObject);
var s,wpath:string;
    i:integer;
    s_info,s1,s2:pchar;
begin
...
procedure TKSFolderBrowserForm.tbBurnClick(Sender: TObject);
var s:string;
    i:integer;
    s_info,s1:pchar;
begin
...
procedure TKSFolderBrowserForm.tbBTClick(Sender: TObject);
var s:string;
    i:integer;
    s_info,s1,s2:pchar;
begin
...
procedure TKSFolderBrowserForm.MenuWinampClick(Sender: TObject);
begin
 tbWinampClick(sender);
end;


procedure TKSFolderBrowserForm.MenuBurnClick(Sender: TObject);
begin
 tbBurnClick(sender);
end;


procedure TKSFolderBrowserForm.MenuBTClick(Sender: TObject);
begin
 tbBTClick(sender);
end;


procedure TKSFolderBrowserForm.MenuCopyClick(Sender: TObject);
var
...
procedure TKSFolderBrowserForm.MenuSelectAllClick(Sender: TObject);
begin
 ShellListView.SelectAll;
end;


procedure TKSFolderBrowserForm.tbSearchClick(Sender: TObject);
...
function TKSFolderBrowserForm.GoToRandomFileInsideRoot(const filename:string):boolean;
var n:integer;
begin
 Result:=false;
...
function TKSFolderBrowserForm.SearchFileOpFunc(wnd:HWND;cmd:TSearchFileOp;const list:TStringList):boolean;
begin
 Result:=false;
