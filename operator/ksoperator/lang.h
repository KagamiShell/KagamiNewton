...
#define S_CALLOPERATOR            "Ученик зовет учителя"
#define S_CALLOPERATORTEXT        "Ученик за компьютером \"%s\\%s (%s\\%s)\" просит Вас подойти"
#define S_NETBURN                 "Необходимо записать диск ученику"
#define S_NETBURNTEXT             "Ученику \"%s\\%s (%s\\%s)\" необходимо записать диск с образа:\n\n%s\n\nЗапустить программу записи дисков?"
...
#define S_STOP                    "Стоп"
#define S_NETDATARETRIEVED        "Данные получены от %d компьютеров из %d"
...
#define S_FIELD_MACHINEID         "Компьютер"
...
#define S_FIELD_KAGAMIVER         "Версия клиента"
...
#define S_RDNOTINSTALLEDONMACHINE "На удаленном компьютере сервис управления рабочим столом не установлен"
...
#define S_CLIENTUPDATEMAX         "Не рекомендуется обновлять одновременно большое число компьютеров из-за возможной нагрузки сети.\n\nПродолжить?"
#define S_RLBHELP  \
...
"При обновлении ученической части KagamiShell через программу оператора этот режим включается автоматически, т.к. обновление клиента KagamiShell требует перезагрузки.\n" \
"Проверить текущее состояние отката всегда можно по команде \"Текущий статус отката\".\n"
