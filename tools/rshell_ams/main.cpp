...
const char *our_appname = "kshell_ams.exe";
...
{
  WriteRegStr(root,"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run","kshell_ams",our_apppath);
}


void RemoveFromAutorun(HKEY root)
{
  DeleteRegValue(root,"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run","kshell_ams");
}
...
int WINAPI WinMain(HINSTANCE hThisInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdParam,int nCmdShow)
{
  ...
  if ( lstrcmpi(PathFindFileName(our_apppath),our_appname) )
     {
       Err("Файл должен называться kshell_ams.exe");
       return 0;
     }
  ...
  CreateMutex(NULL,FALSE,"_kshell_ams.exe_Mutex");
