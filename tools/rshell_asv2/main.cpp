...
const char *our_appname = "kshell_asv2.exe";
...
void G_OnForceUpdate()
{
  if ( g_config )
     {
       g_config->ForceSendInfoToKShell();
     }
}
...
{
  WriteRegStr(root,"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run","kshell_asv2",our_apppath);
}


void RemoveFromAutorun(HKEY root)
{
  DeleteRegValue(root,"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run","kshell_asv2");
}
...
int WINAPI WinMain(HINSTANCE hThisInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdParam,int nCmdShow)
{
  ...
  if ( lstrcmpi(PathFindFileName(our_apppath),our_appname) )
     {
       Err("Файл должен называться kshell_asv2.exe");
       return 0;
     }
  ...
  CreateMutex(NULL,FALSE,"_kshell_asv2.exe_Mutex");
