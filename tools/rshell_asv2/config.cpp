...
CConfig::CConfig()
{
  ...
  h_event_recv_data = CreateEvent(NULL,FALSE,FALSE,"kshell_asv2_Event_RecvData");
  b_got_packet = FALSE;
  ...
  PathAppend(filename,"kshell_asv2.ini");
  if ( !IsFileExist(filename) )
     lstrcpy(filename,"kshell_asv2.ini");  // will be in Windows dir
                 ...
                 if ( !IsStrEmpty(m_curr_status) )
                    {
                      if ( GetTickCount() - last_update_time > update_interval*1000 )
                         {
                           m_kshell->SetStatusString(m_curr_status);
  ...
  if ( m_tariffs.size() > 0 )
     {
       ...
       if ( tar == NULL )
          {
            m_kshell->EnableSheets(NULL,TRUE);
          }
       else
          {
            if ( tar->p_show )
               {
                 m_kshell->EnableSheets(NULL,FALSE);
                 
                 const TSTRINGLIST *list = tar->p_show;
                 for ( int n = 0; n < list->size(); n++ )
                     {
                       m_kshell->EnableSheets((*list)[n],TRUE);
                     }
               }
            else
            if ( tar->p_hide )
               {
                 ...
                 for ( int n = 0; n < list->size(); n++ )
                     {
                       m_kshell->EnableSheets((*list)[n],FALSE);
                     }
               }
            else
               {
  ...
  if ( use_vip_login )
     {
       if ( IsStrEmpty(m_curr_user) || !StrCmpNI(m_curr_user,"КОД ",4)/*fix for non-vip users*/ )
          ...
          {
            m_kshell->VipLogin(m_curr_user);
          }
     }
}


void CConfig::ForceSendInfoToKShell()
{
  if ( b_got_packet )
     {
       // status string
